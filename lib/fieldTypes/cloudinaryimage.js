		...
		if (req.body && req.body[paths.select]) {
			...
			if (field.options.publicID) {
				...
				async.forEach(Object.keys(keystone.lists), function(key, callback) {
      				...
      				var pageId = item.page || item._page;

	      			if (key == 'Page') {
						list.model.findById(pageId, function(err, page) {
					  	  	var page = page.key;
						  	var name = req.files[paths.upload].name;
						  	var lang = '';

						  	if (config.current_locale == 'ca-fre') {
						  		lang = 'fre/';
						  	}

						  	uploadOptions.public_id = lang + page + '/' + name.replace(/(.*)\.[^.]+$/, '$1');
