...
List.prototype.expandColumns = function(cols) {
	...
	var getCol = function(def) {
		...
		} else if (list.model[config.current_locale].schema.paths[def.path] || list.model[config.current_locale].schema.virtuals[def.path]) {
...
List.prototype.paginate = function(options, callback) {
	...
	var list = this, model = this.model[config.current_locale];
