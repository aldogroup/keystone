...
List.prototype.expandColumns = function(cols, locale) {

	var locale = locale || config.default_locale;
	...
	var getCol = function(def) {
		...
		} else if (list.model[locale].schema.paths[def.path] || list.model[locale].schema.virtuals[def.path]) {
...
List.prototype.register = function() {
	...
	if (config.broccoli.build == true) {
		this.model[process.env.CURRENT_LOCALE] = keystone.db[process.env.CURRENT_LOCALE].model(this.key, this.schema);
	} else {
		var that = this;

		config.locales.forEach(function (locale) {
			that.model[locale] = keystone.db[locale].model(that.key, that.schema);
		});
	}
...
List.prototype.paginate = function(options, callback) {
	...
	var list = this, model = this.model[options.locale];
