...
function mount(mountPath, parentApp, events) {
	...
	if (this.get('mongo replica set')){
		...
		this.db = [];
		this.db[config.default_locale] = this.mongoose.createConnection(this.get('mongo'));

		config.locales.forEach(function (locale) {
			if (locale != config.default_locale) {
				keystone.db[locale] = keystone.db[config.default_locale].useDb('keystone_' + locale);
			}
		});
		
	}
	
	this.db[config.default_locale].on('error', function(err) {
