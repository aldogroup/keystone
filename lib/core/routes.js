...
function routes(app) {
	...
	var initLocale = function() {
		return function(req, res, next) {
			var config = require('config');
		  	...
		  	if (config.locales.indexOf(req.params.locale) == -1) {
				req.flash('error', 'Locale ' + req.params.locale + ' could not be found.');
	...
	if (this.get('auth') === true) {
		...
		app.all('/keystone/locale/:locale', initLocale(), require('../../routes/views/locale'));
